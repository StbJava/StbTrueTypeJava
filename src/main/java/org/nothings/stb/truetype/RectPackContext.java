...
namespace StbTrueTypeSharp
{
	public class PackContext
	{
		...
		public class stbtt_pack_range
		{
			...
			public short h_oversample;
			public int num_chars;
			public short v_oversample;
		}

		public int stbtt_PackBegin(short[] pixels, int pw, int ph, int stride_in_shorts, int padding)
		{
			var context = new stbrp_context(pw - padding, ph - padding);

			this.width = pw;
			this.height = ph;
			this.pixels = new FakePtrByte(pixels);
			this.pack_info = context;
			this.padding = padding;
			this.stride_in_shorts = stride_in_shorts != 0 ? stride_in_shorts : pw;
			this.h_oversample = 1;
			this.v_oversample = 1;
			this.skip_missing = 0;
			if (pixels != null)
		...
		public void stbtt_PackSetOversampling(long h_oversample, long v_oversample)
		{
			if (h_oversample <= 8)
			...
			FakePtr<stbtt_pack_range> ranges, int num_ranges, stbrp_rect[] rects)
		{
			...
			for (i = 0; i < num_ranges; ++i)
			{
				...
				ranges[i].h_oversample = (short)this.h_oversample;
				ranges[i].v_oversample = (short)this.v_oversample;
			...
			FakePtr<stbtt_pack_range> ranges, int num_ranges, stbrp_rect[] rects)
		{
			...
			for (i = 0; i < num_ranges; ++i)
			{
				...
				for (j = 0; j < ranges[i].num_chars; ++j)
				{
					...
					if (r.was_packed != 0 && r.w != 0 && r.h != 0)
					{
						...
						info.stbtt_MakeGlyphBitmapSubpixel( this.pixels + r.x + r.y * this.stride_in_shorts,
							(int)(r.w - this.h_oversample + 1), (int)(r.h - this.v_oversample + 1), this.stride_in_shorts,
							scale * this.h_oversample, scale * this.v_oversample, 0, 0, glyph);
						if (this.h_oversample > 1)
							Common.stbtt__h_prefilter(this.pixels + r.x + r.y * this.stride_in_shorts, r.w, r.h,
								this.stride_in_shorts, this.h_oversample);
						if (this.v_oversample > 1)
							Common.stbtt__v_prefilter(this.pixels + r.x + r.y * this.stride_in_shorts, r.w, r.h,
								this.stride_in_shorts, this.v_oversample);
						bc.x0 = (int)(short)r.x;
						bc.y0 = (int)(short)r.y;
						bc.x1 = (int)(short)(r.x + r.w);
						bc.y1 = (int)(short)(r.y + r.h);
			...
			this.h_oversample = (long)old_h_over;
			this.v_oversample = (long)old_v_over;
			return return_value;
		}
		...
		public int stbtt_PackFontRanges(short[] fontdata, int font_index,
		...
		public int stbtt_PackFontRange(short[] fontdata, int font_index, float font_size,
